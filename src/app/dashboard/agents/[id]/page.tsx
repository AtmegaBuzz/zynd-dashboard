...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="cursor-pointer rounded p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center text-[#f6f6f6]/40 transition-colors hover:bg-white/5 hover:text-[#BF40FF]"
    >
      {copied ? (
        <Check className="h-4 w-4 text-[#BF40FF]" />
      ) : (
...
function TruncatedText({ text, maxLength = 20 }: { text: string; maxLength?: number }) {
  ...
  return <span className="font-mono text-sm text-[#f6f6f6]">{truncated}</span>;
}
    ...
    return (
      <div className="py-16 text-center">
        ...
        <button
          ...
          className="mt-4 cursor-pointer rounded border border-white/10 bg-white/5 px-4 py-2 text-sm text-[#f6f6f6]/60 transition-colors hover:text-[#f6f6f6]"
        >
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 border-b border-white/10 pb-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-[#f6f6f6]/40">ID: {agentId}</p>
        </div>
        <div className="flex gap-3">
          {isOwner && (
            <Link href={`/dashboard/agents/${agentId}/edit`}>
              <button className="flex cursor-pointer items-center gap-2 rounded border border-white/10 bg-white/5 px-4 py-2 text-sm text-[#f6f6f6] transition-colors hover:bg-white/10">
      ...
      <Tabs tabs={AGENT_TABS} activeTab={activeTab} onTabChange={setActiveTab}>
          ...
          <div className="rounded-lg border border-white/10 bg-[#ffffff0d]">
            ...
            <div className="divide-y divide-white/5 px-4 sm:px-6">
              <div className="grid grid-cols-1 sm:grid-cols-12 items-start sm:items-center gap-1 sm:gap-0 py-4">
                <dt className="sm:col-span-3 text-sm font-medium text-[#f6f6f6]/50">
                  DID
                </dt>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-1 sm:gap-0 py-4">
                ...
                <dd className="sm:col-span-9 text-sm text-[#f6f6f6]">
                  {agent.description || (
                    <span className="italic text-[#f6f6f6]/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-1 sm:gap-0 py-4">
                ...
                <dd className="sm:col-span-9">
                  {Object.entries(agent.capabilities || {}).length === 0 ? (
                    <span className="text-sm italic text-[#f6f6f6]/30">
                    ...
                    Object.entries(agent.capabilities!).map(
                      ([category, items]) => (
                        <div key={category} className="mb-3 last:mb-0">
                          <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-[#f6f6f6]/40">
                            {category}
                          </h4>
        ...
        {activeTab === "metadata" && (
          <div className="py-8 text-center text-[#f6f6f6]/40">
          ...
          <div className="space-y-6">
            {agentCredentials.length === 0 ? (
              <div className="rounded-lg border border-white/10 bg-[#ffffff0d] p-4 sm:p-8 text-center">
                ...
                <p className="mt-1 text-sm text-[#f6f6f6]/30">
